    ...
    <table class="table w-full border-separate border-spacing-2">
        ...
        <thead class="bg-base-200">
            <tr class="shadow">
                ...
                <th>Jumlah User</th>
                <th class="sticky right-0 bg-base-200 z-[1]">Aksi</th>
            </tr>
        </thead>
        <tbody>
            @forelse ($dataPeran as $role)
                <tr class="shadow hover:bg-base-300">
                    <th class="w-1">
                        <label>
                            <input type="checkbox" class="checkbox" />
                        </label>
                    ...
                    <td>
                        <div class="text-sm opacity-50">{{ $role->guard_name }}</div>
                    </td>
                    <td>
                    ...
                    <td class="sticky right-0 bg-base-300 z-10">
                        <div class="dropdown dropdown-left dropdown-center">
                            ...
                            <ul tabindex="0"
                                ...
                                <li>
                                    <a href="{{ route('role.edit', $role->id) }}" class="text-yellow-500">Edit</a>
                                </li>
                                <li>
                                    <button class="text-red-500 hover:text-red-700 focus:outline-none"
                                        @click="$dispatch('confirm-delete', { get_id: '{{ $role->id }}', data: '{{ $role->name }}' })">
                                        Hapus
                                    </button>
                                </li>
                            </ul>
